# voxtype-codex-dictation — macOS (fn push-to-talk)
...
```bash
~/.local/bin/voxtype-codex-proxy &     # the transcription proxy
~/.local/bin/voxtype-mac               # the fn push-to-talk client
```
...
if via the launch agent, it's **`voxtype-mac`**. After approving, restart:
`launchctl kickstart -k gui/$(id -u)/io.voxtype.mac`.
...
| Var | Default | Notes |
|-----|---------|-------|
| `VOXTYPE_KEYCODE` | `63` (fn) | use another key's keyCode if you don't want fn |
| `VOXTYPE_LANG` | `auto` | force a language, e.g. `en` / `zh` |
| `VOXTYPE_PROXY_URL` | `http://127.0.0.1:8377/v1/audio/transcriptions` | |
| `VOXTYPE_SOX` | auto-detected | full path to `sox` if not on PATH |

Set them in `~/Library/LaunchAgents/io.voxtype.mac.plist`
...
- **fn key**: a global `NSEvent` monitor watches `flagsChanged` for keyCode 63 —
  ...
  set it to **Do Nothing**, or pick a different `VOXTYPE_KEYCODE`.
